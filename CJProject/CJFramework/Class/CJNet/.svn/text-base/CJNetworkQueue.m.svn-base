...
- (void)dealloc
{
	...
	for (CJHTTPRequest *request in [self operations]) {
...
- (void)resetProgressDelegate:(id *)progressDelegate
{
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		double max = 1.0;
		[CJHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&max callerToRetain:nil];
	}
	selector = @selector(setDoubleValue:);
	if ([*progressDelegate respondsToSelector:selector]) {
		double value = 0.0;
		[CJHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#else
...
{
	if ([operation isKindOfClass:[CJHTTPRequest class]]) {
		
		CJHTTPRequest *request = (CJHTTPRequest *)operation;
...
// Only add CJHTTPRequests to this queue!!
- (void)addOperation:(NSOperation *)operation
{
	if (![operation isKindOfClass:[CJHTTPRequest class]]) {
		[NSException raise:@"AttemptToAddInvalidRequest" format:@"Attempted to add an object that was not an CJHTTPRequest to an CJNetworkQueue"];
	}
	...
	if ([self showAccurateProgress]) {
		...
		if ([[request requestMethod] isEqualToString:@"GET"]) {
			if ([self isSuspended]) {
				CJHTTPRequest *HEADRequest = [request HEADRequest];
...
- (void)request:(CJHTTPRequest *)request didReceiveBytes:(long long)bytes
{
	...
	if ([self downloadProgressDelegate]) {
		[CJHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}
...
- (void)request:(CJHTTPRequest *)request didSendBytes:(long long)bytes
{
	...
	if ([self uploadProgressDelegate]) {
		[CJHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}
...
{
	CJNetworkQueue *newQueue = [[[self class] alloc] init];

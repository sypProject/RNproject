...
//#import "CJAuthenticationDialog.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "CJInputStream.h"
#import "CJDataDecompressor.h"
#import "CJDataCompressor.h"

// Automatically set on build
NSString *CJHTTPRequestVersion = @"v1.8.1-61 2011-09-19";

static NSString *defaultUserAgent = nil;

NSString* const NetworkRequestErrorDomain = @"CJHTTPRequestErrorDomain";

static NSString *CJHTTPRequestRunLoopMode = @"CJHTTPRequestRunLoopMode";
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((CJHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
static NSError *CJRequestCancelledError;
static NSError *CJRequestTimedOutError;
static NSError *CJAuthenticationError;
static NSError *CJUnableToCreateRequestError;
static NSError *CJTooMuchRedirectionError;
...
unsigned long const CJWWANBandwidthThrottleAmount = 14800;
...
// When using CJAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <CJCacheDelegate> defaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int runningRequestCount = 0;

// You can use [CJHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(CJHTTPRequest *)theRequest;
...
- (void)performBlockOnMainThread:(CJBasicBlock)block;
- (void)releaseBlocksOnMainThread;
+ (void)releaseBlocks:(NSArray *)blocks;
- (void)callBlock:(CJBasicBlock)block;
#endif
...
@property (assign) CJAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) CJHTTPRequest *PACFileRequest;
...
+ (void)initialize
{
	if (self == [CJHTTPRequest class]) {
		...
		CJRequestTimedOutError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:CJRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];  
        CJAuthenticationError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:CJAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		CJRequestCancelledError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:CJRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		CJUnableToCreateRequestError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:CJUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		CJTooMuchRedirectionError = [[NSError alloc] initWithDomain:NetworkRequestErrorDomain code:CJTooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setShouldAttemptPersistentConnection:YES];//保持连接
	[self setPersistentConnectionTimeoutSeconds:60.0];//保持连接时间，太长没用，和服务器相关
	[self setShouldPresentCredentialsBeforeChallenge:YES];//在服务器要求之前提供证书
	[self setShouldRedirect:YES];//是否跳转
	[self setShowAccurateProgress:YES];//是否显示详细的进度，就是是否有一个连续的进入显示
	[self setShouldResetDownloadProgress:YES];
	[self setShouldResetUploadProgress:YES];
	[self setAllowCompressedResponse:YES];//gzip压缩
	[self setShouldWaitToInflateCompressedResponses:YES];//请求完成对gzip响应包的解压缩
	[self setDefaultResponseEncoding:NSISOLatin1StringEncoding];
	[self setShouldPresentProxyAuthenticationDialog:NO];//不用用户对话框
	
	[self setTimeOutSeconds:[CJHTTPRequest defaultTimeOutSeconds]];
	[self setUseSessionPersistence:YES];
	[self setUseCookiePersistence:YES];
	[self setValidatesSecureCertificate:YES];//安全证书验证
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <CJCacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:CJUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <CJCacheDelegate>)cache andCachePolicy:(CJCachePolicy)policy
{
	CJHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:CJNoAuthenticationNeededYet];
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![CJDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [CJDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
- (void)cancelOnRequestThread
{
	...
	CJ_DEBUG_LOG(@"[STATUS] Request cancelled: %@",self);
#endif
	...
	[self failWithError:CJRequestCancelledError];
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [CJDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	CJ_DEBUG_LOG(@"[STATUS] Starting synchronous request %@",self);
#endif
	[self setSynchronous:YES];
	[self setRunLoopMode:CJHTTPRequestRunLoopMode];
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	CJ_DEBUG_LOG(@"[STATUS] Starting asynchronous request %@",self);
#endif
...
- (void)main
{
	@try {
		...
		if ([CJHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		// A HEAD request generated by an CJNetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
		...
		if (![self url]) {
			[self failWithError:CJUnableToCreateRequestError];
			return;		
		}
		...
		//If this is a HEAD request generated by an CJNetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
			[[self mainRequest] buildRequestHeaders];
		}
		
		// Even if this is a HEAD request with a mainRequest, we still need to call to give subclasses a chance to add their own to HEAD requests (CJS3Request does this)
		...
		if ([self downloadCache]) {
			...
			if ([self cachePolicy] & (CJAskServerIfModifiedWhenStaleCachePolicy|CJAskServerIfModifiedCachePolicy)) {
		...
		NSError *underlyingError = [NSError errorWithDomain:NetworkRequestErrorDomain code:CJUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)applyAuthorizationHeader
{
	...
	if (![self shouldPresentCredentialsBeforeChallenge]) {
		...
		CJ_DEBUG_LOG(@"[AUTH] Request %@ will not send credentials to the server until it asks for them",self);
#endif
		return;
	}
...
#if DEBUG_HTTP_AUTHENTICATION
			CJ_DEBUG_LOG(@"[AUTH] Request %@ has a username and password set, and was manually configured to use BASIC. Will send credentials without waiting for an authentication challenge",self);	
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
							CJ_DEBUG_LOG(@"[AUTH] Request %@ found cached credentials (%@), will reuse without waiting for an authentication challenge",self,[credentials objectForKey:@"AuthenticationScheme"]);
#endif
						} else {
							[[self class] removeAuthenticationCredentialsFromSessionStore:[credentials objectForKey:@"Credentials"]];
#if DEBUG_HTTP_AUTHENTICATION
							CJ_DEBUG_LOG(@"[AUTH] Failed to apply cached credentials to request %@. These will be removed from the session store, and this request will wait for an authentication challenge",self);
#endif
						}
...
#if DEBUG_HTTP_AUTHENTICATION
						CJ_DEBUG_LOG(@"[AUTH] Request %@ found cached BASIC credentials from a previous request. Will send credentials without waiting for an authentication challenge",self);
#endif
					}
				}
			}
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!userAgentString) {
			userAgentString = [CJHTTPRequest defaultUserAgentString];
		}
		if (userAgentString) {
...
- (void)updatePartialDownloadSize
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [fileManager fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[CJInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[CJInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[CJInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[CJInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[NSMakeCollectable(CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream])) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[CJHTTPRequest expirePersistentConnections];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		if ([self connectionInfo]) {
			...
			if (![[[self connectionInfo] objectForKey:@"host"] isEqualToString:[[self url] host]] || ![[[self connectionInfo] objectForKey:@"scheme"] isEqualToString:[[self url] scheme]] || [(NSNumber *)[[self connectionInfo] objectForKey:@"port"] intValue] != [[[self url] port] intValue]) {
				...
				CJ_DEBUG_LOG(@"[CONNECTION] Not re-using connection #%i because it has expired",[[[self connectionInfo] objectForKey:@"id"] intValue]);
#endif
...
#if DEBUG_PERSISTENT_CONNECTIONS
                CJ_DEBUG_LOG(@"%@ - Not re-using connection #%i for request #%i because it is already used by request #%i",self,[[[self connectionInfo] objectForKey:@"id"] intValue],[[self requestID] intValue],[[[self connectionInfo] objectForKey:@"request"] intValue]);
#endif
                [self setConnectionInfo:nil];
            }
		}
...
#if DEBUG_PERSISTENT_CONNECTIONS
		CJ_DEBUG_LOG(@"[CONNECTION] Request #%@ will use connection #%i",[self requestID],[[[self connectionInfo] objectForKey:@"id"] intValue]);
#endif
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("CJStreamID"), [[self connectionInfo] objectForKey:@"id"]);
        
	} else {
#if DEBUG_PERSISTENT_CONNECTIONS
		CJ_DEBUG_LOG(@"[CONNECTION] Request %@ will not use a persistent connection",self);
#endif
	}
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[CJHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[CJHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)performRedirect
{
	...
	if ([self redirectCount] > RedirectionLimit) {
		...
		[self failWithError:CJTooMuchRedirectionError];
		[self setComplete:YES];
	} else {
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		[self failWithError:CJRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[CJHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
                
#if DEBUG_REQUEST_STATUS
				if ([self totalBytesSent] == [self postLength]) {
					CJ_DEBUG_LOG(@"[STATUS] Request %@ finished uploading data",self);
				}
#endif
			}
		}
...
// Used by CJNetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (CJHTTPRequest *)HEADRequest
{
	CJHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[CJHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[CJHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[CJHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[CJHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];
    
	[CJHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[CJHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[CJHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[CJHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[CJHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[CJHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[CJHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[CJHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[CJHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[CJHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[CJHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        NSMethodSignature *cbSignature = [CJHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[CJHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	CJ_DEBUG_LOG(@"[STATUS] Request finished: %@",self);
#endif
	if ([self error] || [self mainRequest]) {
		return;
	}
...
{
#if DEBUG_REQUEST_STATUS || DEBUG_THROTTLING
	CJ_DEBUG_LOG(@"[STATUS] Request %@: %@",self,(theError == CJRequestCancelledError ? @"Cancelled" : @"Failed"));
#endif
	[self setComplete:YES];
	
	// Invalidate the current connection so subsequent requests don't attempt to reuse it
	if (theError && [theError code] != CJAuthenticationErrorType && [theError code] != CJTooMuchRedirectionErrorType) {
		[connectionsLock lock];
#if DEBUG_PERSISTENT_CONNECTIONS
		CJ_DEBUG_LOG(@"[CONNECTION] Request #%@ failed and will invalidate connection #%@",[self requestID],[[self connectionInfo] objectForKey:@"id"]);
#endif
	...
	// If we have cached data, use it and ignore the error when using CJFallbackToCacheIfLoadFailsCachePolicy
	if ([self downloadCache] && ([self cachePolicy] & CJFallbackToCacheIfLoadFailsCachePolicy)) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
	...
	CJHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an CJNetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
...
#if DEBUG_REQUEST_STATUS
	if ([self totalBytesSent] == [self postLength]) {
		CJ_DEBUG_LOG(@"[STATUS] Request %@ received response headers",self);
	}
#endif		
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:CJHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:CJProxyAuthenticationNeeded];
	} else {
#if DEBUG_HTTP_AUTHENTICATION
		if ([self authenticationScheme]) {
			CJ_DEBUG_LOG(@"[AUTH] Request %@ has passed %@ authentication",self,[self authenticationScheme]);
		}
#endif
	}
...
#if DEBUG_HTTP_AUTHENTICATION
			CJ_DEBUG_LOG(@"[AUTH] Request %@ passed BASIC authentication, and will save credentials in the session store for future use",self);
#endif
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[CJHTTPRequest addSessionCookie:cookie];
		}
	}
		...
		if (![httpVersion isEqualToString:(NSString *)kCFHTTPVersion1_0] || [connectionHeader isEqualToString:@"keep-alive"]) {
			...
			if (![connectionHeader isEqualToString:@"close"]) {
				...
				if (keepAliveHeader) { 
...
#if DEBUG_PERSISTENT_CONNECTIONS
                        CJ_DEBUG_LOG(@"[CONNECTION] Got a keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
#endif					
					}
					...
					[self setConnectionCanBeReused:YES];
#if DEBUG_PERSISTENT_CONNECTIONS
                    CJ_DEBUG_LOG(@"[CONNECTION] Got no keep-alive header, will keep this connection open for %f seconds", [self persistentConnectionTimeoutSeconds]);
#endif
				}
			}
		}
...
- (BOOL)willRedirect
{
	...
	// Note that CJHTTPRequest does not currently support 305 Use Proxy
...
#if DEBUG_REQUEST_STATUS
	CJ_DEBUG_LOG(@"[STATUS] Request will redirect (code: %i): %@",responseCode,self);
#endif
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[CJHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[CJHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	CJHTTPRequest *theRequest = [self mainRequest];
	// If this is a HEAD request generated by an CJNetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [CJHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
...
#if DEBUG_HTTP_AUTHENTICATION
		CJ_DEBUG_LOG(@"[AUTH] Request %@ will use credentials set on its url",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			CJ_DEBUG_LOG(@"[AUTH] Request %@ will use credentials from its parent request",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			CJ_DEBUG_LOG(@"[AUTH] Request %@ will use username and password properties as credentials",self);
#endif
		}		
	}
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [CJHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
#if DEBUG_HTTP_AUTHENTICATION
			if (user && pass) {
				CJ_DEBUG_LOG(@"[AUTH] Request %@ will use credentials from the keychain",self);
			}
#endif
		}
	}
...
{
#if DEBUG_HTTP_AUTHENTICATION
    CJ_DEBUG_LOG(@"[AUTH] Request %@ received credentials from its delegate or an CJAuthenticationDialog, will retry",self);
#endif
...
{
#if DEBUG_HTTP_AUTHENTICATION
    CJ_DEBUG_LOG(@"[AUTH] Request %@ had authentication cancelled by its delegate or an CJAuthenticationDialog",self);
#endif
...
- (void)failAuthentication
{
	[self failWithError:CJAuthenticationError];
}
...
- (BOOL)showProxyAuthenticationDialog
{
	...
	if ([self shouldPresentProxyAuthenticationDialog]) {
		//[CJAuthenticationDialog performSelectorOnMainThread:@selector(presentAuthenticationDialogForRequest:) withObject:self waitUntilDone:[NSThread isMainThread]];
		return YES;
	}
	return NO;
#else
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		if ((([self proxyAuthenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self proxyAuthenticationRetryCount] < 2) && [self applyProxyCredentials:proxyCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == CJProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
	...
	if (!requestAuthentication) {
#if DEBUG_HTTP_AUTHENTICATION
		CJ_DEBUG_LOG(@"[AUTH] Request %@ failed to read authentication information from response headers",self);
#endif
    ...
    if ([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM || [self authenticationRetryCount] == 0) {
        CJ_DEBUG_LOG(@"[AUTH] Request %@ received 401 challenge and must authenticate using %@%@",self,[self authenticationScheme],realm);
    } else {
        CJ_DEBUG_LOG(@"[AUTH] Request %@ NTLM handshake step %i",self,[self authenticationRetryCount]+1);
    }
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			CJ_DEBUG_LOG(@"[AUTH] Request %@ had bad credentials, will remove them from the session store if they are cached",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
				CJ_DEBUG_LOG(@"[AUTH] Request %@ failed or was cancelled while waiting to access credentials",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
					CJ_DEBUG_LOG(@"[AUTH] Request %@ will reuse cached credentials from the session (%@)",self,[credentials objectForKey:@"AuthenticationScheme"]);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
				CJ_DEBUG_LOG(@"[AUTH] Request %@ will ask its delegate for credentials to use",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
				CJ_DEBUG_LOG(@"[AUTH] Request %@ will ask CJAuthenticationDialog for credentials",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
		CJ_DEBUG_LOG(@"[AUTH] Request %@ has no credentials to present and must give up",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			CJ_DEBUG_LOG(@"[AUTH] Request %@ has failed NTLM authentication",self);
#endif
...
#if DEBUG_HTTP_AUTHENTICATION
			CJ_DEBUG_LOG(@"[AUTH] Request %@ had credentials and they were not marked as bad, but we got a 401 all the same.",self);
#endif
            
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
#if DEBUG_HTTP_AUTHENTICATION
				CJ_DEBUG_LOG(@"[AUTH] Request %@ failed to apply credentials",self);
#endif
				[delegateAuthenticationLock unlock];
				[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
			}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[CJHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
    ...
    if (bytesRead < 0) {
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[CJDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[CJHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
{	
...
#if DEBUG_REQUEST_STATUS
	CJ_DEBUG_LOG(@"[STATUS] Request %@ finished downloading data (%qu bytes)",self, [self totalBytesRead]);
#endif
	[self setStatusTimer:nil];
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[CJDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:NetworkRequestErrorDomain code:CJFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
...
#if DEBUG_PERSISTENT_CONNECTIONS
	if ([self requestID]) {
		CJ_DEBUG_LOG(@"[CONNECTION] Request #%@ finished using connection #%@",[self requestID], [[self connectionInfo] objectForKey:@"id"]);
	}
#endif
	[[self connectionInfo] removeObjectForKey:@"request"];
	[[self connectionInfo] setObject:[NSDate dateWithTimeIntervalSinceNow:[self persistentConnectionTimeoutSeconds]] forKey:@"expires"];
	...
	if (![self needsRedirect] && ![self authenticationNeeded] && ![self didUseCachedResponse]) {
        ...
        // If request has asked delegate or CJAuthenticationDialog for credentials
...
- (void)useDataFromCache
{
	...
	if (headers && dataPath) {
		...
		[self setResponseStatusCode:[[headers objectForKey:@"X-CJHTTPRequest-Response-Status-Code"] intValue]];
...
#if DEBUG_PERSISTENT_CONNECTIONS
        CJ_DEBUG_LOG(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - will retry with a new connection", [[self connectionInfo] objectForKey:@"id"]);
#endif
		[connectionsLock lock];
		[[self connectionInfo] removeObjectForKey:@"request"];
		[persistentConnectionsPool removeObject:[self connectionInfo]];
...
#if DEBUG_PERSISTENT_CONNECTIONS
    CJ_DEBUG_LOG(@"[CONNECTION] Request attempted to use connection #%@, but it has been closed - we have already retried with a new connection, so we must give up", [[self connectionInfo] objectForKey:@"id"]);
#endif	
	return NO;
}
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:NetworkRequestErrorDomain code:CJFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	// For file:// urls, we'll use an async NSInputStream (CJHTTPRequest does not support file:// urls)
	...
	// Create an CJHTTPRequest to fetch the PAC file
	CJHTTPRequest *PACRequest = [CJHTTPRequest requestWithURL:[self PACurl]];
...
#if DEBUG_PERSISTENT_CONNECTIONS
			CJ_DEBUG_LOG(@"[CONNECTION] Closing connection #%i because it has expired",[[existingConnection objectForKey:@"id"] intValue]);
#endif
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	CJHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
+ (NSMutableArray *)sessionCookies
{
	...
	if (!sessionCookies) {
		[CJHTTPRequest setSessionCookies:[NSMutableArray array]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[CJHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[CJHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[CJHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[CJHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:CJCacheForSessionDurationCacheStoragePolicy];
}
...
+ (NSString *)defaultUserAgentString
{
	@synchronized (self) {
		...
		if (!defaultUserAgent) {
			...
			// If we couldn't find one, we'll give up (and CJHTTPRequest will use the standard CFNetwork user agent)
			if (!appName) {
				return nil;
			}
            ...
            // We won't bother to check for systems prior to 10.4, since CJHTTPRequest only works on 10.5+
            OSErr err;
...
- (void)performThrottling
{
	...
	[CJHTTPRequest measureBandwidthUsage];
		...
		if (throttleWakeUpTime) {
			if ([throttleWakeUpTime timeIntervalSinceDate:[NSDate date]] > 0) {
				if ([self readStreamIsScheduled]) {
					[self unscheduleReadStream];
#if DEBUG_THROTTLING
					CJ_DEBUG_LOG(@"[THROTTLING] Sleeping request %@ until after %@",self,throttleWakeUpTime);
#endif
				}
			} else {
				if (![self readStreamIsScheduled]) {
					[self scheduleReadStream];
#if DEBUG_THROTTLING
					CJ_DEBUG_LOG(@"[THROTTLING] Waking up request %@",self);
#endif
				}
			}
		} 
...
#if DEBUG_THROTTLING
	CJ_DEBUG_LOG(@"[THROTTLING] ===Used: %lu bytes of bandwidth in last measurement period===",bandwidthUsedInLastSecond);
#endif
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[CJHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an CJNetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[CJHTTPRequest throttleBandwidthForWWANUsingLimit:CJWWANBandwidthThrottleAmount];
	} else {
		[CJHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[CJHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[CJHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[CJHTTPRequest reachabilityChanged:nil];
}
...
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [CJHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <CJCacheDelegate>)cache
{
	@synchronized (self) {
		[cache retain];
...
+ (id <CJCacheDelegate>)defaultCache
{
    @synchronized(self) {
...
+ (NSThread *)threadForRequest:(CJHTTPRequest *)request
{
...
+ (NSDate *)expiryDateForRequest:(CJHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [CJHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
#pragma mark blocks
#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(CJBasicBlock)aStartedBlock
{
	[startedBlock release];
	startedBlock = [aStartedBlock copy];
}

- (void)setHeadersReceivedBlock:(CJHeadersBlock)aReceivedBlock
{
	[headersReceivedBlock release];
	headersReceivedBlock = [aReceivedBlock copy];
}

- (void)setCompletionBlock:(CJBasicBlock)aCompletionBlock
{
	[completionBlock release];
	completionBlock = [aCompletionBlock copy];
}

- (void)setFailedBlock:(CJBasicBlock)aFailedBlock
{
	[failureBlock release];
	failureBlock = [aFailedBlock copy];
}

- (void)setBytesReceivedBlock:(CJProgressBlock)aBytesReceivedBlock
{
	[bytesReceivedBlock release];
	bytesReceivedBlock = [aBytesReceivedBlock copy];
}

- (void)setBytesSentBlock:(CJProgressBlock)aBytesSentBlock
{
	[bytesSentBlock release];
	bytesSentBlock = [aBytesSentBlock copy];
}

- (void)setDownloadSizeIncrementedBlock:(CJSizeBlock)aDownloadSizeIncrementedBlock{
	[downloadSizeIncrementedBlock release];
	downloadSizeIncrementedBlock = [aDownloadSizeIncrementedBlock copy];
}

- (void)setUploadSizeIncrementedBlock:(CJSizeBlock)anUploadSizeIncrementedBlock
{
	[uploadSizeIncrementedBlock release];
	uploadSizeIncrementedBlock = [anUploadSizeIncrementedBlock copy];
}

- (void)setDataReceivedBlock:(CJDataBlock)aReceivedBlock
{
	[dataReceivedBlock release];
	dataReceivedBlock = [aReceivedBlock copy];
}

- (void)setAuthenticationNeededBlock:(CJBasicBlock)anAuthenticationBlock
{
	[authenticationNeededBlock release];
	authenticationNeededBlock = [anAuthenticationBlock copy];
}
- (void)setProxyAuthenticationNeededBlock:(CJBasicBlock)aProxyAuthenticationBlock
{
	[proxyAuthenticationNeededBlock release];
	proxyAuthenticationNeededBlock = [aProxyAuthenticationBlock copy];
}
- (void)setRequestRedirectedBlock:(CJBasicBlock)aRedirectBlock
{

...
@class CJDataDecompressor;

extern NSString *CJHTTPRequestVersion;
...
typedef enum _CJAuthenticationState {
	CJNoAuthenticationNeededYet = 0,
	CJHTTPAuthenticationNeeded = 1,
	CJProxyAuthenticationNeeded = 2
} CJAuthenticationState;

typedef enum _CJNetworkErrorType {
    CJConnectionFailureErrorType = 1,
    CJRequestTimedOutErrorType = 2,
    CJAuthenticationErrorType = 3,
    CJRequestCancelledErrorType = 4,
    CJUnableToCreateRequestErrorType = 5,
    CJInternalErrorWhileBuildingRequestType  = 6,
    CJInternalErrorWhileApplyingCredentialsType  = 7,
	CJFileManagementError = 8,
	CJTooMuchRedirectionErrorType = 9,
	CJUnhandledExceptionError = 10,
	CJCompressionError = 11
	
} CJNetworkErrorType;


// The error domain that all errors generated by CJHTTPRequest use
...
extern unsigned long const CJWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^CJBasicBlock)(void);
typedef void (^CJHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^CJSizeBlock)(long long size);
typedef void (^CJProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^CJDataBlock)(NSData *data);
#endif

@interface CJHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the CJHTTPRequestDelegate protocol
	id <CJHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but CJNetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <CJHTTPRequestDelegate, CJProgressDelegate> queue;
	...
	// Automatically set to true in CJFormDataRequests when using setFile:forKey:
	...
	// Set to true when CJHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = CJConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <CJProgressDelegate> uploadProgressDelegate;
	...
	id <CJProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force CJHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, CJHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, CJHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - CJHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	// Tells CJHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let CJHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// CJHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See CJAuthenticationState values above. 0 == default == No authentication needed yet
	CJAuthenticationState authenticationNeeded;
	
	// When YES, CJHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops CJHTTPRequest cleaning up a temporary post body)
	...
	// Will be CJHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [CJHTTPRequest setDefaultCache:cache] to configure a default cache
	id <CJCacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See CJCacheDelegate.h for possible values
	CJCachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See CJCacheDelegate.h for possible values
	CJCacheStoragePolicy cacheStoragePolicy;
	...
	CJDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using CJHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
...
// Used internally to record bandwidth use, and by CJInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, CJHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) CJHTTPRequest *mainRequest;
...
@property (assign, readonly) CJAuthenticationState authenticationNeeded;
...
@property (assign) id <CJCacheDelegate> downloadCache;
@property (assign) CJCachePolicy cachePolicy;
@property (assign) CJCacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) CJDataDecompressor *dataDecompressor;

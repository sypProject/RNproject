...
#import "CJFormDataRequest.h"

// Private stuff
@interface CJFormDataRequest ()
...
@implementation CJFormDataRequest
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPostFormat:CJURLEncodedPostFormat];
...
- (void)addFile:(NSString *)filePath withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if (!fileExists || isDirectory) {
		[self failWithError:[NSError errorWithDomain:NetworkRequestErrorDomain code:CJInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",filePath],NSLocalizedDescriptionKey,nil]]];
	}
	...
	if (!contentType) {
		contentType = [CJHTTPRequest mimeTypeForFileAtPath:filePath];
	}
...
- (void)buildPostBody
{
	...
	if ([self postFormat] == CJURLEncodedPostFormat) {
...
#if DEBUG_FORM_DATA_REQUEST
	CJ_DEBUG_LOG(@"%@",[self debugBodyString]);
...
- (void)buildURLEncodedPostBody
{
	...
	if ([[self fileData] count] > 0) {
		[self setPostFormat:CJMultipartFormDataPostFormat];
		[self buildMultipartFormDataPostBody];
		return;
	}
...
{
	CJFormDataRequest *newRequest = [super copyWithZone:zone];
